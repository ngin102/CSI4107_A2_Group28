...
def main():
    ...
    results_use_experiment = os.path.join(
        current_file_dir, "results_experiment1_GPTNeo.txt"
    )
    ...
    if os.path.exists(results_use_experiment):
        print("Existing results GPT3 txt file detected. This file will be deleted.")
    ...
    for key in queries.get_queries().keys():
        ...
        search.calculateSimilarity(
            queries.get_queries().get(key), use_cls=gpt3Retrieval
        )
        gpt3Retrieval.clearVariables()
